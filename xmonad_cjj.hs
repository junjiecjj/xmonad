...
myStartupHook = do
                  ...
                  spawn "ps cax | grep  flameshot; if ! [ $? -eq 0 ]; then   flameshot >/dev/null 2>&1 &; fi"
                  spawn "ps cax | grep  dunst; if ! [ $? -eq 0 ]; then  dunst; fi"
                  spawn "ps cax | grep  copyq; if ! [ $? -eq 0 ]; then  copyq ; fi"
                  spawn "ps cax | grep  pasystray; if ! [ $? -eq 0 ]; then  pasystray  >/dev/null 2>&1 &; fi"
                  spawn "ps cax | grep  kmix; if ! [ $? -eq 0 ]; then  kmix   >/dev/null 2>&1 &; fi"
                  spawn "ps cax | grep  pa-applet; if ! [ $? -eq 0 ]; then /foo/bar/bin/pa-applet   >/dev/null 2>&1 &; fi"
                  spawn "ps cax | grep  mictray; if ! [ $? -eq 0 ]; then   mictray   >/dev/null 2>&1 &; fi"
                  -- spawn "nm-applet &"
